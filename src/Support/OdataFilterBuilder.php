...
class OdataFilterBuilder
{
     ...
     *   When false (default, OData v4 spec-compliant), GUID literals are emitted
     *   as bare Edm.Guid values: field eq 08EC1E80-...
     *   When true, GUIDs are kept as quoted strings: field eq '08EC1E80-...'
     *   Set to true for servers (e.g. FileMaker) whose OData parser cannot handle
     *   bare GUIDs in $filter expressions.
     */
    ...
    public function build(array $conditions): string
    {
        ...
        foreach ($conditions as $condition) {
            if ($condition->isOperator) {
                ...
                $parts[] = $this->convertCondition($condition->expr);
            }
        }

        return $this->applyBooleanPrecedence($parts);
    }
     ...
     */
    private function applyBooleanPrecedence(array $parts): string
    {
    ...
    /**
     * Formats a filter literal for OData v4 syntax.
     *
     * Dates and datetimes are unquoted (typed literals in OData v4).
     * GUIDs are unquoted by default per the OData v4 spec (Edm.Guid literals).
     * Pass quoteGuids: true to the constructor to keep them quoted — required
     * by some servers (e.g. FileMaker) that cannot parse bare GUID tokens.
     * Plain strings stay quoted; numeric/null literals are returned as-is.
     */
    private function formatFilterValue(string $value): string
    {
        if ((str_starts_with($value, "'") && str_ends_with($value, "'"))
            ...
            if (preg_match(self::DATE_PATTERN, $inner)
                || preg_match(self::DATETIME_PATTERN, $inner)
            ) {
                return $inner;
            }

            if (!$this->quoteGuids && preg_match(self::GUID_PATTERN, $inner)) {
                return $inner;
            }
        }
    ...
    private function convertCondition(string $expr): string
    {
        ...
        if (preg_match('/^(.+?)\s+IN\s*\((.+)\)\s*$/i', $expr, $m)) {
            $col     = self::formatIdentifier($m[1]);
            $values  = array_map(fn($v) => $this->formatFilterValue(trim($v)), $this->splitInValues($m[2]));
        ...
        while ($i < $len) {
            ...
            foreach (self::OPERATOR_MAP as $sql => $odata) {
                if (substr($expr, $i, strlen($sql)) === $sql) {
                    ...
                    $right = $this->formatFilterValue(trim(substr($expr, $i + strlen($sql))));
